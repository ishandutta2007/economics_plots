...
ax.plot(years, mumbai_index, color='#1f77b4', linewidth=3, 
        label=f'Mumbai (25-Yr Growth: {m_25_year_factor:.1f}x)')
ax.plot(years, kolkata_index, color='#ff7f0e', linewidth=3, 
...
ax.set_yscale('log')
ax.set_title('Recalibrated 50-Year Real Estate Index Trend (Base 100 in 1976)\nAdjusted for the 2002-2008 Transactional Super-Cycle', 
...
ax.axvline(x=2001, color='purple', linestyle='--', alpha=0.6, label='25 Years Ago Benchmark (2001)')
ax.axvline(x=2008, color='red', linestyle=':', alpha=0.6, label='2008 Global Financial Crisis')

# Data value anchors for the year 2026
ax.text(2026, mumbai_index[-1], f'  {int(mumbai_index[-1]):,}\n  (x{int(mumbai_index[-1]/100)})', 
        color='#1f77b4', fontweight='bold', va='center', fontsize=10)
ax.text(2026, kolkata_index[-1], f'  {int(kolkata_index[-1]):,}\n  (x{int(kolkata_index[-1]/100)})', 
        color='#ff7f0e', fontweight='bold', va='center', fontsize=10)

# Grid and Legend configurations
ax.grid(True, which="both", linestyle=':', alpha=0.5)
ax.legend(loc='upper left', frameon=True, facecolor='#ffffff', edgecolor='#e0e0e0', fontsize=11)
