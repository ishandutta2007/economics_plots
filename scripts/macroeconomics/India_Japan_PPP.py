...
india_ppp = np.array([
    5629,  6048,  6592,  7001,  6802,  7840,  8965,  9880,  10747, 11789, 
    12801, 13819, 14837, 15957, 17175, 18485, 19871, 21361, 22963, 24685, 26536
])

japan_ppp = np.array([
    40941, 42422, 43198, 43886, 43703, 46312, 50439, 52928, 54369, 56854, 
    59207, 61213, 62957, 64844, 66841, 68887, 70953, 73081, 75273, 77531, 79856
])
...
ax.plot(years[:11], japan_ppp[:11], color='#BC002D', linestyle='-', marker='o', linewidth=2.5, label='Japan PPP (Historical)')
...
ax.plot(years[10:], japan_ppp[10:], color='#BC002D', linestyle=':', marker='o', linewidth=2.5, label='Japan PPP (Projected)')
...
for i in range(len(years)):
    ...
    ax.annotate(f"${japan_ppp[i]:,}", (years[i], japan_ppp[i]), textcoords="offset points", 
                xytext=(0, 10), ha='center', fontsize=8, color='#80001C', weight='bold')
...
for idx in gap_years:
    ax.annotate('', xy=(years[idx], india_ppp[idx]), xytext=(years[idx], japan_ppp[idx]),
    ...
    ax.text(years[idx] + 0.2, mid_y, f"Gap:\n{multiples[idx]}", color='#4B0082', 
            weight='bold', fontsize=11, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))

# Formatting chart features
ax.set_title('Macroeconomic Catch-Up: India vs. Japan GDP per Capita (PPP) Progression', fontsize=15, weight='bold', pad=20)
