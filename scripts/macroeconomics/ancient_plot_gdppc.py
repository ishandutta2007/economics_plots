...
data_path = os.path.join(
    script_dir, "..", "..", "data", source + "_Europe_India_China_PPP.csv"
)
...
def plot_gdp():
    ...
    plt.plot(
        df["Year"],
        df["Europe GDP/Capita"],
        label="Europe GDP per capita",
        marker="o",
        linewidth=2,
    )
    plt.plot(
    ...
    for i in range(len(df)):
        plt.annotate(
            str(df["Europe GDP/Capita"].iloc[i]),
            (df["Year"].iloc[i], df["Europe GDP/Capita"].iloc[i]),
        ...
        plt.annotate(
            str(df["India GDP/Capita"].iloc[i]),
            (df["Year"].iloc[i], df["India GDP/Capita"].iloc[i]),
            textcoords="offset points",
            xytext=(0, 5),
            ha="center",
            fontsize=8,
        ...
        plt.annotate(
            str(df["China GDP/Capita"].iloc[i]),
            (df["Year"].iloc[i], df["China GDP/Capita"].iloc[i]),
            textcoords="offset points",
            xytext=(0, 5),
            ha="center",
            fontsize=8,
