...
df_combined = (
    pd.merge(df_us, df_in, on="Year", how="outer")
    .sort_values("Year")
...
ax.plot(
    df_in["Year"],
    df_in["India_Vehicles_Per_Capita"],
    color="#ff7f0e",
    linestyle="-",
    marker="s",
    linewidth=2.5,
...
for i, txt in enumerate(df_us["US_Vehicles_Per_Capita"]):
    if df_us["Year"].iloc[i] in [
        ...
        ax.annotate(
            f"{txt:.2f}",
            (df_us["Year"].iloc[i], df_us["US_Vehicles_Per_Capita"].iloc[i]),
...
for i, txt in enumerate(df_in["India_Vehicles_Per_Capita"]):
    if df_in["Year"].iloc[i] in [
        ...
        ax.annotate(
            f"{txt:.3f}",
            (df_in["Year"].iloc[i], df_in["India_Vehicles_Per_Capita"].iloc[i]),
            textcoords="offset points",
            xytext=(0, -16),
            ha="center",
            fontsize=9,
...
ax.set_title(
    "Comparative Motorization Trajectory: US vs. India (Vehicles Per Capita)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
