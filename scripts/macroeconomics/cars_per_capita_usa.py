...
ax1.plot(
    df["Year"],
    df["Registered_Vehicles_Millions"],
    color=color_veh,
    linestyle="-",
    marker="s",
...
for i, txt in enumerate(df["Vehicles_Per_Capita"]):
    ax2.annotate(
        f"{txt:.2f}",
        (df["Year"].iloc[i], df["Vehicles_Per_Capita"].iloc[i]),
        ...
        weight="bold",
        bbox=dict(
            boxstyle="round,pad=0.3", fc="white", ec=color_cap, lw=0.5, alpha=0.8
        ),
    )
...
for i, txt in enumerate(df["Registered_Vehicles_Millions"]):
    ax1.annotate(
        f"{txt:.1f}M",
        (df["Year"].iloc[i], df["Registered_Vehicles_Millions"].iloc[i]),
        textcoords="offset points",
        xytext=(-10, -15),
        ha="center",
        fontsize=9,
...
# Title and legends
plt.title(
    "U.S. Population vs. Registered Vehicles & Vehicles Per Capita (1910-2024)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
