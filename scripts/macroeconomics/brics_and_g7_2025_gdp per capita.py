...
fig.suptitle('GDP Per Capita Comparison: G7 vs BRICS (USD)', fontsize=16, fontweight='bold', y=0.98)

# 3. Plot G7 Countries (Left Plot)
g7_color = '#1f77b4'  # Deep Blue
ax1.barh(list(g7_sorted.keys()), list(g7_sorted.values()), color=g7_color, edgecolor='black', height=0.6)
...
for index, value in enumerate(g7_sorted.values()):
    ax1.text(value + 1000, index, f"${value:,}", va='center', fontsize=10, fontweight='semibold')

# 4. Plot BRICS Countries (Right Plot)
brics_color = '#d62728'  # Crimson Red
ax2.barh(list(brics_sorted.keys()), list(brics_sorted.values()), color=brics_color, edgecolor='black', height=0.6)
...
for index, value in enumerate(brics_sorted.values()):
    ax2.text(value + 500, index, f"${value:,}", va='center', fontsize=10, fontweight='semibold')
