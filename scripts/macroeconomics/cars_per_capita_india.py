...
vehicles_per_capita = [
    v / p for v, p in zip(registered_vehicles_millions, population_millions)
]
...
ax1.plot(
    df_india["Year"],
    df_india["India_Population_Millions"],
    color=color_pop,
    linestyle="--",
    marker="o",
    alpha=0.7,
    label="India Population (Millions)",
)
ax1.plot(
    df_india["Year"],
    df_india["Registered_Vehicles_Millions"],
    color=color_veh,
    linestyle="-",
    marker="s",
    alpha=0.8,
    label="Registered Vehicles (Millions)",
)
...
ax2.plot(
    df_india["Year"],
    df_india["Vehicles_Per_Capita"],
    color=color_cap,
    linestyle="-.",
    marker="^",
    linewidth=2.5,
    label="Vehicles Per Capita",
)
ax2.set_ylabel(
    "Vehicles Per Capita", color=color_cap, fontsize=12, fontweight="bold", labelpad=15
)
...
for i, txt in enumerate(df_india["Vehicles_Per_Capita"]):
    ax2.annotate(
        f"{txt:.3f}",
        (df_india["Year"].iloc[i], df_india["Vehicles_Per_Capita"].iloc[i]),
        ...
        weight="bold",
        bbox=dict(
            boxstyle="round,pad=0.2", fc="white", ec=color_cap, lw=0.5, alpha=0.9
        ),
    )
...
for i, txt in enumerate(df_india["Registered_Vehicles_Millions"]):
    ax1.annotate(
        f"{txt:.1f}M",
        (df_india["Year"].iloc[i], df_india["Registered_Vehicles_Millions"].iloc[i]),
        textcoords="offset points",
        xytext=(0, -18),
        ha="center",
        fontsize=8,
...
plt.title(
    "India Population Scale vs. Vehicle Motorization Trajectory (1951-2025)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
