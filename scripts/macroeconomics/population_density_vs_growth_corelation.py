...
def load_indicator(filename):
    ...
    if filename == LAND_FILE:
        keep = ["Country Name", "Country Code", END_YEAR]
...
df = pop.merge(gdp, on=["Country Name", "Country Code"], suffixes=("_POP", "_GDP"))

df = df.merge(land, on=["Country Name", "Country Code"])

df.rename(columns={END_YEAR: "LandArea"}, inplace=True)
...
df["Growth"] = (df["GDP2024"] - df["GDP1990"]) / df["GDP1990"] * 100
...
for ax, (label, low, high) in zip(axes, bands):
    sub = df[(df.Pop2024 >= low) & (df.Pop2024 < high)]
    ...
    ax.set_title(f"{label}\nr={result.rvalue:.2f}, R²={result.rvalue**2:.2f}")
    ...
    for _, row in sub.iterrows():
        ax.text(
            np.log10(row["Density"]), row["Growth"], row["Country Code"], fontsize=7
        )

plt.tight_layout()

plt.savefig("population_density_vs_growth.png", dpi=300)
...
summary.to_csv("density_regression_summary.csv", index=False)

###############################################################################
# SAVE PROCESSED DATA
###############################################################################

df.to_csv("processed_country_data.csv", index=False)
