...
bars_2025 = ax.barh(
    y_pos + bar_width / 2, gdp_2025, bar_width, label="2025", color="#1f77b4", alpha=0.9
)
bars_2050 = ax.barh(
    y_pos - bar_width / 2, gdp_2050, bar_width, label="2050", color="#ff7f0e", alpha=0.9
)
...
ax.axvspan(
    ...
    alpha=0.2,
    color="orange",
    label="2050s Middle Income levels($20K-$60K)",
)
ax.axvspan(
    60000,
    max(gdp_2050) + 25000,
    alpha=0.2,
    color="green",
    label="2050s Developed levels($60K+)",
)
...
ax.set_title(
    "GDP per Capita Comparison: 2025 vs 2050 Projections",
    fontsize=14,
    fontweight="bold",
)
ax.set_yticks(y_pos)
...
for i, (val_2025, val_2050) in enumerate(zip(gdp_2025, gdp_2050)):
    ...
    ax.text(
        val_2050 + 2000,
        i - bar_width / 2,
        f"${val_2050 / 1000:.0f}K ({(val_2050 / val_2025):.0f}x)",
        va="center",
        fontweight="bold",
        fontsize=10,
    )
