...
brics_ppp = np.array([
    11800, 12550, 13320, 14050, 13720, 14900, 16180, 17450, 18720, 19950,
    21250, 22600, 24010, 25500, 27080, 28750, 30510, 32370, 34330, 36400, 38580
])

g7_ppp = np.array([
    53400, 55800, 57900, 59700, 57400, 61800, 65400, 68900, 72100, 75200,
    78300, 81400, 84600, 87900, 91300, 94800, 98400, 102100, 105900, 109800, 113800
])
...
ax.plot(years[:11], brics_ppp[:11], color='#A21C26', linestyle='-', marker='o', linewidth=2.5, label='BRICS PPP Average (Historical)')
ax.plot(years[:11], g7_ppp[:11], color='#1D4ED8', linestyle='-', marker='o', linewidth=2.5, label='G7 PPP Average (Historical)')

# 2. Plot Projection Lines (2026 to 2036 - Indices 10 to 20)
ax.plot(years[10:], brics_ppp[10:], color='#A21C26', linestyle=':', marker='o', linewidth=2.5, label='BRICS PPP Average (Projected)')
ax.plot(years[10:], g7_ppp[10:], color='#1D4ED8', linestyle=':', marker='o', linewidth=2.5, label='G7 PPP Average (Projected)')
...
for i in range(len(years)):
    ...
    ax.annotate(f"${brics_ppp[i]:,}", (years[i], brics_ppp[i]), textcoords="offset points", 
                xytext=(0, -15), ha='center', fontsize=8, color='#781017', weight='bold')
    # G7 Labels (offset above the trajectory marker)
    ax.annotate(f"${g7_ppp[i]:,}", (years[i], g7_ppp[i]), textcoords="offset points", 
                xytext=(0, 10), ha='center', fontsize=8, color='#112E82', weight='bold')
...
for idx in gap_years:
    ax.annotate('', xy=(years[idx], brics_ppp[idx]), xytext=(years[idx], g7_ppp[idx]),
    ...
    ax.text(years[idx] + 0.2, mid_y, f"Gap:\n{multiples[idx]}", color='#581C87', 
            weight='bold', fontsize=11, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))

# Graphical standard formatting
ax.set_title('Global Economic Shift: G7 vs. BRICS Average GDP per Capita (PPP) Catch-up', fontsize=15, weight='bold', pad=20)
