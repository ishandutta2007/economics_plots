...
def plot_gdp():
    ...
    for i in range(len(df)):
        plt.annotate(
            str(df["Europe GDP ($B)"].iloc[i]),
            (df["Year"].iloc[i], df["Europe GDP ($B)"].iloc[i]),
        ...
        plt.annotate(
            str(df["India GDP ($B)"].iloc[i]),
            (df["Year"].iloc[i], df["India GDP ($B)"].iloc[i]),
            textcoords="offset points",
            xytext=(0, 5),
            ha="center",
            fontsize=8,
        ...
        plt.annotate(
            str(df["China GDP ($B)"].iloc[i]),
            (df["Year"].iloc[i], df["China GDP ($B)"].iloc[i]),
            textcoords="offset points",
            xytext=(0, 5),
            ha="center",
            fontsize=8,
    ...
    plt.title(
        f"Historical GDP (PPP): Europe vs India vs China (source:{source})", fontsize=16
    )
