...
plt.plot(
    ...
    marker="o",
    color="royalblue",
    label="Western Europe (Historical)",
    linewidth=2,
)
plt.plot(
    years,
    ...
    marker="s",
    color="darkorange",
    label="Eastern Europe (Historical)",
    linewidth=2,
)
...
plt.plot(
    ...
    alpha=0.7,
    linewidth=2,
    label="Western Europe (Projection)",
)
plt.plot(
    future_years,
    ee_future,
    ...
    alpha=0.7,
    linewidth=2,
    label="Eastern Europe (Projection)",
)

# Highlight the convergence intersection point
convergence_value = np.exp(slope_we * conv_year + intercept_we)
plt.scatter(
    conv_year, convergence_value, color="crimson", zorder=5, s=120, edgecolors="black"
)
plt.annotate(
...
plt.yscale("log")
plt.title(
    "GDP per Capita Trend & Convergence Projection (Log Scale)",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
