...
for label, style in professions.items():
    plt.plot(df['Year'], df[label], marker=style['marker'], color=style['color'], linewidth=2, label=label)
...
plt.plot(df['Year'], df['Core Inflation Curve'], color='red', linestyle='--', linewidth=3, label='Core Inflation Curve (Base Indexed)')
...
plt.title('30-Year Trend of Mean Indian Labor Wages vs. Inflation (1995 - 2026)', fontsize=16, fontweight='bold', pad=20)
