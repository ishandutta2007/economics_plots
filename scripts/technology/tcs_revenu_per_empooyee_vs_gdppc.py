...
ax.plot(
    ...
    color=color_ias,
    linewidth=2,
    label="IAS Officer Starting Salary",
)
ax.plot(
    years,
    tcs_starting_salary_usd,
...
plt.title(
    "TCS Performance & Salaries vs. IAS Starting Salary & India GDP per Capita (Log Scale: 1997 - 2026)",
    fontsize=14,
    fontweight="bold",
    pad=25,
)
...
for i, (year, rev, gdp, sal, ias_sal) in enumerate(
    zip(
        years,
        tcs_rev_per_emp,
        india_gdp_pc,
...
ax.annotate(
    ...
    xytext=(1997, tcs_rev_per_emp[0]),
    arrowprops=dict(arrowstyle="<->", linestyle=":", color="#333333", linewidth=2),
)
ax.text(
    1997 + 0.3,
    (tcs_starting_salary_usd[0] * tcs_rev_per_emp[0]) ** 0.5,
...
ax.annotate(
    ...
    xytext=(2026, tcs_rev_per_emp[-1]),
    arrowprops=dict(arrowstyle="<->", linestyle=":", color="#333333", linewidth=2),
)
ax.text(
    2026 - 0.3,
    (tcs_starting_salary_usd[-1] * tcs_rev_per_emp[-1]) ** 0.5,
