...
line1 = ax1.plot(
    ...
    marker="o",
    linewidth=2.5,
    label="Total Aspirants (Log Scale)",
)
line2 = ax1.plot(
    years,
    seats,
    ...
    marker="s",
    linewidth=2.5,
    label="Total IIT Seats (Log Scale)",
)
...
for i, txt in enumerate(seats):
    ax1.annotate(
        ...
        color="#2980b9",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#3498db", alpha=0.85),
    )
...
for i, txt in enumerate(acceptance_rate):
    ax2.annotate(
        ...
        color="#27ae60",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#2ecc71", alpha=0.85),
    )

# 5. Labels and Grid Formatting
ax1.set_xlabel("Year", fontsize=12, fontweight="bold", labelpad=10)
ax1.set_ylabel(
    "Number of People (Seats / Aspirants)",
    color="#2c3e50",
    fontsize=12,
    fontweight="bold",
)
...
plt.title(
    "70-Year Evolution of IIT Admissions: Seats, Aspirants & Selectivity (1956–2026)",
    fontsize=14,
    fontweight="bold",
    pad=15,
)
