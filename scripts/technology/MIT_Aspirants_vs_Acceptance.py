...
years = [2000, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026]

# Total admitted students (approximate incoming class size + yield buffer)
seats = [1729, 1519, 1511, 1452, 1464, 1427, 1457, 1340, 1337, 1259, 1275, 1334, 1299]
...
line1 = ax1.plot(
    ...
    marker="o",
    linewidth=2.5,
    label="Total Applicants (Log Scale)",
)  # MIT Red
line2 = ax1.plot(
    years,
    seats,
    ...
    marker="s",
    linewidth=2.5,
    label="Total Admitted (Log Scale)",
)  # MIT Silver Gray
...
for i, txt in enumerate(aspirants):
    ...
    ax1.annotate(
        ...
        color="#A31F34",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#A31F34", alpha=0.85),
    )
...
for i, txt in enumerate(seats):
    ...
    ax1.annotate(
        ...
        color="#8A8B8C",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#8A8B8C", alpha=0.85),
    )
...
for i, txt in enumerate(acceptance_rate):
    ax2.annotate(
        ...
        color="#2c3e50",
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#2c3e50", alpha=0.85),
    )

# 5. Labels and Grid Formatting
ax1.set_xlabel("Year", fontsize=12, fontweight="bold", labelpad=10)
ax1.set_ylabel(
    "Number of People (Admitted / Applicants)",
    color="#2c3e50",
    fontsize=12,
    fontweight="bold",
)
...
ax1.axvspan(
    ...
    color="#A31F34",
    alpha=0.05,
    label="Test-Optional & Pandemic Era (2020-2022)",
)

# 6. Title and Legend Layout
plt.title(
    "25-Year Evolution of MIT Admissions: Admitted, Applicants & Selectivity (2000–2026)",
    fontsize=14,
    fontweight="bold",
    pad=15,
)
