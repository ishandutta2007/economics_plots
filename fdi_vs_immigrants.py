...
def find_fdi_table(tables_list):
    ...
    for table in tables_list:
        ...
        if isinstance(cols, pd.MultiIndex):
             if ('Country/Territory', 'Country/Territory') in cols and ('% of GDP', '2023') in cols:
                 print("Found FDI table.")
                 df_fdi = table.copy()
                 df_fdi.columns = ['_'.join(col).strip() for col in df_fdi.columns.values]
                 df_fdi = df_fdi[['Country/Territory_Country/Territory', '% of GDP_2023']]
...
def find_immigrant_table(tables_list):
    ...
    for table in tables_list:
        cols = table.columns
        if not isinstance(cols, pd.MultiIndex) and 'Name' in cols and '% of population' in cols:
...
try:
    ...
    if not merged_imm.empty:
        ...
        plt.scatter(merged_imm['Immigrant_pct_Pop'], merged_imm['FDI_pct_GDP'], alpha=0.7)
    ...
    print("Failed to generate plots. Check URLs and page structures, which may have changed.")
    traceback.print_exc()
