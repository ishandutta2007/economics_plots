...
popt_ai, pcov_ai = curve_fit(
    exponential_func,
    x_fit_common,
    ai_energy_hist_values,
    p0=[ai_energy_hist_values[0], 0.4],
    maxfev=5000,
)
a_ai, b_ai = popt_ai
print(
    f"AI Energy Fitted Parameters (10yr hist): a={a_ai:.2f}, b={b_ai:.4f}, CAGR: {(np.exp(b_ai) - 1) * 100:.2f}%"
)
...
popt_total, pcov_total = curve_fit(
    exponential_func,
    x_fit_common,
    total_electricity_hist_values,
    p0=[total_electricity_hist_values[0], 0.03],
    maxfev=5000,
)
a_total, b_total = popt_total
print(
    f"Total Electricity Fitted Parameters (10yr hist): a={a_total:.2f}, b={b_total:.4f}, CAGR: {(np.exp(b_total) - 1) * 100:.2f}%"
)
...
for year in years_full_range:
    ...
    if year <= years_historical[-1]:
        ...
        total_val = (
            total_electricity_hist_values[hist_idx[0]] if len(hist_idx) > 0 else np.nan
        )
    else:
...
print(
    f"Projected Total Electricity in 2050 (10yr hist extrap): {total_electricity_full[-1]:,.0f} TWh"
)
...
ax1.set_ylabel(
    "Energy (TWh) [Log Scale]", color="black", fontsize=14
)  # Single Y-axis label
...
(line_total,) = ax1.plot(
    [], [], lw=2.5, color=color_total, label="Total Electricity Generation (Line)"
)
(line_ai,) = ax1.plot(
    [],
    [],
    lw=2.5,
    color=color_ai,
    label="AI Energy Consumption (Line - Extrap.)",
    linestyle="--",
)
...
year_text = ax1.text(
    0.5, 1.02, "", transform=ax1.transAxes, fontsize=20, fontweight="bold", ha="center"
)

# Set Y-axis limits for the single log scale
min_val_overall = np.min(
    ai_energy_full[~np.isnan(ai_energy_full) & (ai_energy_full > 0)]
)
...
fig.legend(
    loc="upper center", bbox_to_anchor=(0.65, 0.17), ncol=1, fontsize=18
)  # Combined legend
plt.title(
    "Global Energy vs AI Energy",
    fontsize=22,
    pad=45,
    color="darkred",
    fontweight="bold",
...
def animate(i):
    ...
    line_total.set_data(
        x_line_data[valid_total_line], y_total_line_data[valid_total_line]
    )
...
ani = animation.FuncAnimation(
    fig,
    animate,
    frames=len(years_full_range) + pause_frames,
    init_func=init,
    blit=False,
    interval=200,
    repeat=False,
