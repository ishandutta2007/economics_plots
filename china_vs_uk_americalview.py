...
def generate_china_uk_gdp_plot():
    ...
    china_growth_rate = 0.040         # 4.0% average annual growth in real GDP per capita (long-term estimate)
    uk_growth_rate = 0.009            # 0.9% average annual growth in real GDP per capita (long-term estimate)
    start_year = 2025
    
    # Calculate convergence year (around 2051 based on these rates)
    # The script logic calculates the exact convergence point
    years_to_converge = round(np.log(current_gdp_uk_ppp / current_gdp_china_ppp) / np.log((1 + china_growth_rate) / (1 + uk_growth_rate)))
    ...
    plt.plot(years, gdp_china_data, label=f'China GDP/Capita (Growth: {china_growth_rate*100:.1f}%)', color='red', linewidth=2)
